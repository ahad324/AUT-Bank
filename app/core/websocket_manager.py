...
class ConnectionManager:
    ...
    async def send_personal_message(self, message: dict, entity_id: int, connection_type: str):
        print(f"Attempting to send message to {connection_type} {entity_id}")
        print(f"Current connections state: {self.active_connections}")
        
        try:
            if connection_type in self.active_connections and entity_id in self.active_connections[connection_type]:
                ...
                for connection in connections:
                    try:
                        ...
                        print(f"Successfully sent message to {connection_type} {entity_id}")
                    except Exception as e:
    ...
    async def broadcast(self, message: dict):
        ...
        for connection_type in self.active_connections:
            for entity_id, connections in self.active_connections[connection_type].items():
                for connection in connections.copy():  # Use copy to avoid modification during iteration
                    try:
                        ...
                        print(f"Broadcast error to {connection_type} {entity_id}: {str(e)}")
    ...
    async def disconnect(self, websocket: WebSocket, entity_id: int, connection_type: str):
